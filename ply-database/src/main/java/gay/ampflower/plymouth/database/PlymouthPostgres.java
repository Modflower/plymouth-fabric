...
import static gay.ampflower.plymouth.database.DatabaseHelper.toJson;
...
public class PlymouthPostgres extends PlymouthSQL implements Plymouth {
    ...
    private final Int2IntMap blocks = new Int2IntOpenHashMap(32);
    private final Int2IntMap worlds = new Int2IntOpenHashMap(32);
    private final Int2IntMap players = new Int2IntOpenHashMap(32);
    private final Int2IntMap entities = new Int2IntOpenHashMap(32);
    private PreparedStatement
            ...
            getModificationsAt, getModificationsAtDuring, getModificationsAtBy, getModificationsAtDuringBy,
    ...
    public void initializeDatabase() throws PlymouthException {
        ...
        try {
            ...
            getModificationsAt = connection.prepareStatement("SELECT * FROM blocks WHERE (pos).x = ? AND (pos).y = ? AND (pos).z = ?;");
            getModificationsAtBy = connection.prepareStatement("SELECT * FROM blocks WHERE cause_id = ? AND (pos).x = ? AND (pos).y = ? AND (pos).z = ?;");
            getModificationsAtDuring = connection.prepareStatement("SELECT * FROM blocks WHERE time > ? AND time < ? AND (pos).x = ? AND (pos).y = ? AND (pos).z = ?;");
            getModificationsAtDuringBy = connection.prepareStatement("SELECT * FROM blocks WHERE time > ? AND time < ? AND cause_id = ? AND (pos).x = ? AND (pos).y = ? AND (pos).z = ?;");
        ...
        return areStatesUnnecessary(state) ? getBlockIndex(state == null ? null : state.getBlock()) :
                blocks.computeIfAbsent(state.hashCode(), $ -> {
                    try {
                        ...
                        getElseInsertBlock.setObject(2, toJson(state.getEntries(), DatabaseHelper.bannedProperties, Property::getName, Objects::toString));
                        var i = getElseInsertBlock.executeQuery();
                        if (!i.next()) throw new SQLException("?!");
                        return i.getInt(1);
                    } catch (SQLException sql) {
    ...
    protected int getUserIndex(DamageSource entity) throws PlymouthException {
        return players.computeIfAbsent(DatabaseHelper.getHash(entity), $ -> {
            try {
                getElseInsertUser.setObject(1, DatabaseHelper.getName(entity));
                getElseInsertUser.setObject(2, UUIDHelper.getUUID(entity));
